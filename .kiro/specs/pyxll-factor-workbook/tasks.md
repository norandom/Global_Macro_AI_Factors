...
- 6.1/validation remediation (2026-07-09): S1 count-check semantics corrected — compares TOTAL gathered rows per MAIN arm vs n_per_class (the screen's gather invariant; included counts vary with recorded attrition) and never checks the parse_sample arm; verified live: all five step views now build against the real data-v1 release with EVERY check ok (new opt-in test test_live_all_five_step_views_agree_on_full_data). fw_step also renders SchemaError as the canonical #ERROR string (R1.4 leg). Known accepted: 5 registry entries (decision_log_v2, equity_v2, targets_v2, nonpit_scores, macro_panel) are loadable-but-unconsumed audit surface; test_docs.py was added by 5.3 (accepted boundary extension); root uv.lock is the mechanical companion of the dev-group change.
...
- 7.1: S0 ships 36 checks (2 windows x [7 metrics + 5 SSR fields + 2 crisis episodes x 3 fields]) — exact full-data agreement since build_static_bh.py wrote the stats with the same rederive/vendored functions. Generated workbook default tag is now data-v2; sheet order Index,S0,S1..S5. S0_FRAMING embeds the caveat + SSR 0.147 as static copies (accepted 5.2-style pattern; re-sync if build_static_bh.py rewords). Live-verified against the published data-v2 (test_live_s0_agrees_on_full_data_v2). Parquet.Document is unavailable on many Excel builds — S0.md documents the CSV fallback incl. German-locale typing.
