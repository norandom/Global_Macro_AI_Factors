...
| **R2** gates + PASS/FAIL verdict | `compute_ssr`; `ContrastResult.contamination_premium`; `equity_metrics` (Calmar/DD) | A single verdict object combining skill-t>2, SSR stability via `ssr_inference` (one-sided MBB p<0.05; SSR reported as effect size), recall≈0, no-Calmar/tail-regression | **Missing (orchestration)** |
...
3. **Absolute gate thresholds may be unreachable.** Current strategies sit at **SSR≈0.14** on ~10y; the post-cutoff OOS window is only ~1.5y (~15 monthly rebalances) — thin for a HAC t or for rolling-Sharpe inference. Design should reconsider whether R2's gates are absolute or **relative + significance-of-improvement**, and whether a longer/rolling or block-bootstrap evaluation is required for statistical power. _This is the single most important feasibility risk._ **(Resolved for the stability gate, 2026-07-23:** the block bootstrap is now the rule, not an option — `ssr_inference` runs a one-sided moving-block bootstrap on the mean rolling Sharpe at `alpha=0.05` and the SSR is reported as an effect size only. The old `SSR≥1.96` bar is retired: SSR carries no √n, so comparing it to a z critical value demanded a mean rolling Sharpe of ~18–23 and nothing could pass. See `docs/ssr_verdict_review.md`. The `t>2` skill gate and the thin-OOS power concern stand.)
...
- The gap analysis flagged that the literal R2 gates may be structurally unreachable (basket residual ≈1.2%; SSR≈0.14 today; ~1.5y OOS). The design keeps them as the **default `absolute` thresholds** (satisfying R2 verbatim) and adds a configurable `GateConfig.mode="relative_improvement"` plus effect-size/CI reporting as the escape hatch. This is an implementation configuration, not a requirements change; the default verdict still encodes the requirement. Flagged as Open Question 1–2 for the tasks phase to set the loop's default mode. **(Amended 2026-07-23:** the stability half of that pair is no longer `SSR≥1.96`. Acceptance criterion 2.3 now reads: one-sided moving-block-bootstrap p < `GateConfig.ssr_alpha` (default 0.05) on the mean rolling Sharpe, with mean rolling Sharpe > `sr_star`; SSR is the reported effect size and is never thresholded. `ssr_inference` is the single verdict authority — `skill_metric._stability_gate` delegates to `ssr.stable` and raises if `GateConfig.ssr_alpha != ssr.alpha`. `verdict()` also separates a decisively negative sample ("stably BELOW") from a genuinely inconclusive one ("luck-compatible"). Never re-encode the rule or threshold a bare SSR. Skill gate `t>2` is unchanged.)
